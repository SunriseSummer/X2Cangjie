...
```
单文件用例:     186/186  翻译→编译→运行匹配 (100%)
项目级用例:      32/32   翻译→cjpm 编译→运行匹配 (100%)
总计:           218/218  (100%)
```
...
| 新增特性 (161–186) | 26 | 100% | companion object、扩展函数、集合操作、综合 |
| 项目级 (proj_*) | 32 | 100% | 多文件协作：DSA、设计模式、电商、医院等 |
...
| 方向 | 预期收益 | 复杂度 |
|:-----|:---------|:------:|
| 性能优化：heuristics.rs 建立 name→NodeId 索引 | 大型程序翻译提速 | 低 |
| stdlib_map 数据驱动重构 | 减少 render 内联映射，新增 API 只需改表 | 中 |
| render_class 函数拆分 | 可维护性提升 | 低 |
| 支持更多集合操作 (flatMap, groupBy, associate) | 扩大覆盖面 | 低 |
