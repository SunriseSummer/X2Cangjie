...
**接口/抽象类/用户泛型类**/`data class` 自动 `toString`/**open/override 多态**/**companion object 静态方法**）、枚举（含**构造器参数**）、集合、异常、位运算、区间成员检查、`is` 类型判定、解构声明、
`?.` 安全调用、`!!` 非空断言、`maxOf/minOf`、**返回集合的链式高阶（`map/filter/sorted/fold/reduce` 等）**、
**`when (x) { in 区间 -> }` 区间分支**、**字符串与非字符串 `+` 拼接**、**字符级访问与字符算术**、**扩展函数（`extend` 语法）**、可空 `if (x != null)` 智能转换
及常见空安全惯用法等特性，**178 个端到端用例全部翻译 → `cjc` 编译 → 运行输出逐字节匹配（178/178/178）**，
