...
**返回集合的链式高阶（`map/filter/sorted/fold/reduce` 等）**及常见空安全惯用法等特性，
**81 个端到端用例全部翻译 → `cjc` 编译 → 运行输出逐字节匹配（81/81/81）**。
...
| 仓颉 `cjc` 编译通过 | 81/81 |
| 运行输出匹配 | 81/81 |
| 泛化抽样（未训练的新程序） | 累计 13/15 通过（详见 §3.2） |
...
- **类型抽象**：类（主构造器/方法/多类协作/继承）、`sealed class` + `when (x) { is T -> }`、`data class`、`enum class`（+`@Derive[Equatable]` 与自定义 `toString`）、解构声明 `val (a, b) = p`。
...
合计 **13/15 通过**，两类失败（可空流敏感、Option 直接打印）均为已文档化的设计边界（见 §4）。
...
> 注：原表中的「`is` 类型判定」「二进制/十六进制/下划线字面量」「返回集合的链式高阶 `map/filter/reduce/sorted`」已在本轮实现并移出风险表。
...
1. **可空流敏感转换**：把 `if (n == null) return x` 之后的非空使用改写为 `match (n) { case Some(v) => ... }`，
