...
及常见空安全惯用法等特性，**100 个端到端用例全部翻译 → `cjc` 编译 → 运行输出逐字节匹配（100/100/100）**，
其中含 1 个 **1068 行**与 4 个 500–550 行的大规模综合程序（综合工具箱、HR 薪酬、计算器解释器、图算法工具箱）及 3 个 200–310 行综合程序。
...
| 泛化抽样（未训练的新程序） | 累计 19/22 通过（详见 §3.2） |
