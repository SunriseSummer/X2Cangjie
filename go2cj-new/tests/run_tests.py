#!/usr/bin/env python3
"""Test driver for go2cj_new.

Walks ``tests/cases/*.go``, converts each one with the go2cj_new converter,
then:
...
def _convert(go_path: Path, cj_path: Path):
    ...
    res = _run(
        [sys.executable, "-m", "go2cj_new", str(go_path), "-o", str(cj_path), "--report"],
        env=env,
    )
...
def write_log(results):
    n = len(results)
    if not n:
        LOG_PATH.write_text("# go2cj_new test log\n\nNo cases found.\n", encoding="utf-8")
        return
