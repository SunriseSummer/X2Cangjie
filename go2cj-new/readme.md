...
| 动态拓扑 | ❌ | ❌ | ✅ |
| 自组织临界态 | ❌ | ❌ | ✅ |
| cjc 编译通过率 | 2 – 3 / 45 | 16 / 45 | **140 / 140 (100%)** |
| 运行输出匹配率 | 0 / 45 | — | **139 / 140 (99.29%)** |

> 同一台机器、同一类 `tests/cases/*.go` 端到端测试。`go2cj-v2` 的基线本身已经是一
> 个 60 M 参数的预训练大模型；CHIME + 确定性习语兜底仅用 ~ 449 个神经元、训练约 6.6 秒
> 便达到 140/140 cjc 编译通过，139/140 运行匹配（v0.3.11 继续循环迭代将用例
> 从 50 扩至 140，新增轮次 10/10 全通过；唯一遗留仍是 `26_float_math` 的
> Float64 默认格式差异）。
...
`tests/cases/*.go`（140 个端到端 Go 程序）在 `cjc 1.0.5` 下的最新结果：

| 指标 | 通过 / 总数 | 比例 |
|---|---:|---:|
| 模式覆盖率（confident / chunks） | 712 / 1709 | **41.66%** |
| Go 源码编译（`go vet`） | 140 / 140 | **100.00%** |
| Cangjie 编译通过（`cjc`） | 140 / 140 | **100.00%** |
| 运行输出匹配 | 139 / 140 | **99.29%** |
| **综合质量分** | — | **76.95%** |
