...
> 便达到 150/150 cjc 编译通过，149/150 运行匹配（v0.3.12 继续循环迭代将用例
> 从 50 扩至 150，新增轮次 10/10 全通过；唯一遗留仍是 `26_float_math` 的
> Float64 默认格式差异）。
...
`tests/cases/*.go`（150 个端到端 Go 程序）在 `cjc 1.0.5` 下的最新结果：

| 指标 | 通过 / 总数 | 比例 |
|---|---:|---:|
| 模式覆盖率（confident / chunks） | 786 / 1925 | **40.83%** |
| Go 源码编译（`go vet`） | 150 / 150 | **100.00%** |
| Cangjie 编译通过（`cjc`） | 150 / 150 | **100.00%** |
| 运行输出匹配 | 149 / 150 | **99.33%** |
| **综合质量分** | — | **76.60%** |
