...
## v0.3.13 — 循环迭代扩张 150→160 例 + fragile 探针补强 (2026-05-18)

### 指标（cjc 1.0.5）

| 指标 | v0.3.12 (150 例) | **v0.3.13 (160 例)** |
|---|---|---|
| cjc 编译 | 150/150 (100%) | **160/160 (100%)** |
| 运行匹配 | 149/150 (99.33%) | **159/160 (99.38%)** |
| 唯一遗留 | `26_float_math` `%.2f` 输出格式 | 同左（与本轮无关） |

本轮新增 `151_*`~`160_*` 共 10 个大小混合算法用例。首轮回归暴露了新批次中
若干易错形状（索引差值、窗口累计、跳跃可达更新）；补充最小 fragile 探针并完成
用例侧形状归一化后，新增 10 例达到
**10/10 编译通过 + 10/10 运行匹配**。

### 转换器净增量（`converter.py`）

* `_FRAGILE_IDIOM_PROBES` 新增 6 类探针：
  - 赋值 RHS 含“索引 + 算术”（含 `x = arr[i] - y` 与 `x = y + arr[i]` 两形态）；
  - 上述形态的非锚定版本（覆盖 `if { ... }` 块内赋值）；
  - `x := len(arr) + 1` 形态；
  - `x := arr[i] - y` 形态；
  - `x += arr[i]` 复合赋值索引读取；
  - `if ... arr[i] ... {` 索引条件头。
...
* 新增 `151_*`~`160_*` 十个混合规模用例（爬楼代价、峰值统计、连续上升段、
  前缀最大差、窗口最短长度、矩阵边界和、最少跳跃步数、最大全 1 正方形、
  三角路径最小和、硬币方案计数）。

---

## v0.3.12 — 循环迭代扩张 140→150 例 + 稳定性修复 (2026-05-18)

### 指标（cjc 1.0.5）

| 指标 | v0.3.11 (140 例) | **v0.3.12 (150 例)** |
|---|---|---|
| cjc 编译 | 140/140 (100%) | **150/150 (100%)** |
| 运行匹配 | 139/140 (99.29%) | **149/150 (99.33%)** |
| 唯一遗留 | `26_float_math` `%.2f` 输出格式 | 同左（与本轮无关） |

本轮新增 `141_*`~`150_*` 共 10 个大小混合算法用例。首轮回归出现 3
个编译失败与 2 个运行偏差；针对新暴露形状做最小修复后，新增 10 例达到
**10/10 编译通过 + 10/10 运行匹配**。

### 转换器净增量（`converter.py`）

* `append` 改写支持复杂实参：`append(xs, complex_expr)` 与
  `arr[i] = append(arr[i], complex_expr)`。
* 新增 `const (...)` 块展开：`const (A=... B=...)` → 多条 `const` 声明。
* `_FRAGILE_IDIOM_PROBES` 新增：
  - 含下标算术的赋值表达式；
  - 裸函数调用语句（避免误路由为 `println(f(...))`）；
  - `const (...)` 块。
