"""CLI entry point: ``python -m go2cj_new input.go -o output.cj``."""
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        prog="go2cj_new",
        description="Convert a Go source file to Cangjie source using the "
                    "CHIME (Critical Homeostatic Incremental Memory) engine.",
    )
    ...
    if args.report:
        sys.stderr.write(
            f"[go2cj_new] chunks={result.chunks} "
