...
Usage::
    ...
    python -m go2cj_v2.train                     # 1 epoch incremental
    python -m go2cj_v2.train --epochs 3          # multi-epoch
    python -m go2cj_v2.train --restart --epochs 3   # restart from base

Best-checkpoint protocol (mirrors go2cj v1):

* ``go2cj_v2/finetuned/``       — best by ``val_seq_acc``; used for inference.
* ``go2cj_v2/finetuned_last/``  — latest end-of-epoch snapshot for resuming.
* ``go2cj_v2/train_meta.json``  — running epoch counter + best metrics.
...
def main(argv=None):
    p = argparse.ArgumentParser(prog="go2cj_v2.train")
    ...
    p.add_argument("--augment-factor", type=int, default=12)
