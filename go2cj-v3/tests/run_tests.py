#!/usr/bin/env python3
"""Test driver for go2cj_v3.

Walks ``tests/cases/*.go``, converts each one with the go2cj_v3 converter,
then:
...
def _convert(go_path: Path, cj_path: Path):
    ...
    res = _run(
        [sys.executable, "-m", "go2cj_v3", str(go_path), "-o", str(cj_path), "--report"],
        env=env,
    )
...
def write_log(results):
    n = len(results)
    if not n:
        LOG_PATH.write_text("# go2cj_v3 test log\n\nNo cases found.\n", encoding="utf-8")
        return
